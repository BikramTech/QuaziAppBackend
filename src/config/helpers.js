...
const SendErrorsAsResponse = (err, responseCallback, errorMessage) => {
    const errorMessages = err ? ParseModelValidationErrorMessages(err.errors) : "";
...
const SendSuccessResponseWithAuthHeader = (responseCallback, token, response) => {

    return responseCallback.header({
        "x-auth-token": token,
